    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
				...
				o.ambient.rgb = ShadeSH9(half4(o.N, 1));
				o.ambient.a = 1;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				half NdotV = saturate(dot(i.N, -i.V));
				fixed4 glow = pow(1 - NdotV, _GlowPower) * _GlowColor;
				fixed4 lambert = i.NdotL * _LightColor0 * shadow + i.ambient;
				fixed specular = max(dot(reflect(i.V, i.N), _WorldSpaceLightPos0.xyz) * _SpecularMultiplier + _SpecularCompensation, 0) * shadow;
				fixed4 color = _Color * lambert + specular * _LightColor0 + glow;	
